...
describe Queen do
  ...
  before do
    board.positions[4][4] = bqueen
  end

  context 'black queen movement calculations' do
    # Check that the Queen moves as expected when no pieces are in the way
    it 'should move diagonally/laterally from its starting coordinate' do
      bqueen.find_possible_moves(board.positions)
      expect(bqueen.possible_moves.sort).to eq([[0, 0], [0, 4], [1, 1], [1, 4],
                                                [1, 7], [2, 2], [2, 4], [2, 6],
                                                [3, 3], [3, 4], [3, 5], [4, 0],
                                                [4, 3], [4, 5], [4, 6], [4, 7],
                                                [5, 3], [5, 4], [5, 5], [6, 2],
    ...
    it 'should not move through or onto pieces of the same color' do
      board.positions[5][2] = Pawn.new(false, [2, 4])
      board.positions[4][1] = Pawn.new(false, [4, 3])
      bqueen.find_possible_moves(board.positions)
      expect(bqueen.possible_moves.sort).to eq([[0, 0], [0, 4], [1, 1], [1, 4],
                                                [1, 7], [2, 2], [2, 4], [2, 6],
                                                [3, 3], [3, 4], [3, 5], [4, 2],
    ...
    it 'should not move through pieces of the opposing color, but can move onto them' do
      board.positions[5][2] = Pawn.new(true, [2, 4])
      board.positions[4][1] = Pawn.new(true, [4, 3])
      bqueen.find_possible_moves(board.positions)
      expect(bqueen.possible_moves.sort).to eq([[0, 0], [0, 4], [1, 1], [1, 4],
                                                [1, 7], [2, 2], [2, 4], [2, 6],
                                                [3, 3], [3, 4], [3, 5], [4, 1],
